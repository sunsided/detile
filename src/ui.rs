...
impl ViewerApp {
    fn ensure_cache(&mut self, ctx: &egui::Context) {
        let tol_key = (self.tolerance * 10.0).round() as i32;
        let fresh = self
            .cache
            .as_ref()
            .map(|c| c.level == self.selected && c.tolerance_key == tol_key)
            .unwrap_or(false);
        if fresh {
            return;
        }
        let d = &self.levels[self.selected];
        let overlay_img = draw_overlay(&self.source, d).to_rgba8();
        let atlas = build_atlas(&self.source, d, self.tolerance);
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::TopBottomPanel::top("bar").show(ctx, |ui| {
            ui.horizontal_wrapped(|ui| {
                ...
                ui.separator();
                ui.label("←/→ switch config · Tab toggle view");
            });
        });
